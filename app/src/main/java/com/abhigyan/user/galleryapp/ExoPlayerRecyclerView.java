...
import com.abhigyan.user.galleryapp.Activities.VideoViewingActivity;
...
import com.abhigyan.user.galleryapp.GridAdapters.VideoPlayerViewHolder;
...
public class ExoPlayerRecyclerView extends RecyclerView {
    ...
    // ui
    private ImageView thumbnail, volumeControl,moreCtrl;
    ...
    public void playVideo(boolean isEndOfList) {
        ...
        moreCtrl.setOnClickListener(moreClickListener);

        DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(
                context, Util.getUserAgent(context, "RecyclerView VideoPlayer"));
        String mediaUrl = mediaObjects.get(targetPosition).getVideoData();
        Glide.with(context).asDrawable().thumbnail(0.1f).load(mediaObjects.get(targetPosition).getVideoData()).into(thumbnail);
        if (mediaUrl != null) {
    ...
    private OnClickListener moreClickListener = new OnClickListener() {
        ...
        public void onClick(View view) {
            final PopupMenu popup = new PopupMenu(context,moreCtrl);
                ...
                public boolean onMenuItemClick(MenuItem item) {
                    switch (item.getItemId()) {
                        case R.id.play_video:
                            Intent intent = new Intent(context, VideoViewingActivity.class);
                            intent.putExtra("data", mediaObjects.get(playPosition).getVideoData());
                            intent.putExtra("name", mediaObjects.get(playPosition).getVideoUniqueName());
                        ...
                        case R.id.delete_video:
                            boolean confirm = new FileHandler(context).deleteVideoFile(mediaObjects.get(playPosition).getVideoData());
                            if(confirm){
                                Toast.makeText(context, "DONE!", Toast.LENGTH_SHORT).show();
                            }else {
                                Toast.makeText(context, "Failed to remove "+mediaObjects.get(playPosition).getVideoUniqueName(), Toast.LENGTH_SHORT).show();
                            }
                            popup.dismiss();
                            return true;
